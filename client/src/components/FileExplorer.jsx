...
import {FolderOpenIcon, FileTextIcon, FileCodeIcon} from 'lucide-react'
...
function TreeItem({node,activeFile,onFileSelect,depth=0}){
    ...
    if(node.isDir){
        return (
            <div>
                <div className = "flex items-center gap-2 px-3 text-xs text-zinc-400 select-none" style = {{paddingLeft: `${depth * 12 +8}`}}>
...
const FileExplorer = ({files,activeFile,onFileSelect}) => {
  ...
  return (
    <div className = "py-2 overflow-y-auto hide-scrollbar">
        ...
        {tree.length === 0 ? (
            <p className = "px-3 py-2 text-xs text-zinc-400">No files generated yet.</p>
        ) : (
            tree.map((node)=>(
