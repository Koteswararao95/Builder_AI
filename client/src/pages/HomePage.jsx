...
const HomePage = () => {
  ...
  const {user,projects,loadingProjects,generatedProject,handleGenerate,loadProjects,handleDelete,logout} = useAppContext()
      ...
      <div className = 'flex-1 flex flex-col items-center justify-center px-6 pb-20 mt-8 xl:mt-28'>
        <div className = "w-full max-w-2xl flex flex-col items-center">
          ...
          <p className = 'text-center text-sm md:text-base max-w-xl mt-4 text-white/65 leading-relaxed'>
            Describr your idea and watch AI design, structure and launch your website instantly. No coding required.
          </p>
              ...
              {projects.map((p)=>(
                <div key = {p._id} className =  'bg-white/5 border border-white/10 rounded-lg px-4 py-3 flex items-center justify-between group hover:border-white/20 hover:bg-white/10 cursor-pointer backdrop-blur-md transition-all' onClick = {()=>navigate(`/builder/${p._id}`)}>
                      <div className = "flex-1 min-w-0"> 
                        <p className = "text-white font-medium text-white truncate">{p.name}</p>
