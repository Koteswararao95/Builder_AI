...
const AuthPage = ({mode}) => {
    ...
    const handleSubmit = async(e)=>{
        ...
        try{
         ...
         setError(err.message || (mode === "login"? "Invaild email or password": "Registration failed"));
        }
        finally{
            setLoading(false)
    ...
    return (
        ...
        <div className = "flex-1 flex items-center justify-center p-8">
            ...
            {error && <div className = 'mb-6 p-3 border border-reb-200 bg-red-50 text-red-700 text-xs rounded'>{error}</div>}
            ...
            <form className = 'space-y-6' onSubmit ={handleSubmit}>
                {!isLogin &&(
                 <div>
                    ...
                    <input type = "text" value={name} onChange = {(e)=>setName(e.target.value)} required  className = 'w-full pl-2 py-2 border-b border-zinc-200 focus:outline-none focus:border-zinc-950 text-sm text-zinc-900 bg-transparent placeholder-zinc-300 transition-colors' placeholder = 'Jone Doe'/>
                ...
                <div>
                    ...
                    <div  className = 'relative'>
                        <input type = {showPassword ? "text":"password"} value={password} onChange = {(e)=>setPassword(e.target.value)} required  className = 'w-full pl-2 py-2 border-b border-zinc-200 focus:outline-none focus:border-zinc-950 text-sm text-zinc-900 bg-transparent placeholder-zinc-300 transition-colors' placeholder = '••••••••'/>
                ...
                <button type = "submit" disabled = {loading} className = 'w-full py-2.5 bg-linear-to-br from-red-600 to-amber-600 text-white font-sembold hover:scale-102 disabled:opacity-40 flex items-center justify-center cursor-pointer mt-2 rounded-lg transition-all'>
